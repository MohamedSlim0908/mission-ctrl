...
import { formatRuntime, formatTokens, formatDate, getStatusBgClass, getModelColor } from '../utils/format.js'
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/5">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs px-2 py-0.5 rounded border ${statusBg}`}>
            {agent.status}
          </span>
          <span 
            className="text-xs px-2 py-0.5 rounded border"
            style={{ color: modelColor, borderColor: `${modelColor}40`, background: `${modelColor}10` }}
          >
            {agent.model}
          </span>
          {agent.thinkingLevel && agent.thinkingLevel !== 'off' && (
            <span className="text-xs px-2 py-0.5 rounded border border-purple-500/30 text-purple-400 bg-purple-500/10">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {agent.sessionKey && (
          <div className="p-4 border-b border-white/5">
            ...
            <div className="flex items-center gap-2">
              <code className="text-xs text-gray-400 font-mono bg-white/5 px-2 py-1 rounded flex-1 truncate">
                {agent.sessionKey}
              </code>
              <button onClick={copyKey} className="text-gray-500 hover:text-gray-300 p-1">
                <Copy size={14} />
              </button>
            </div>
          </div>
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xs font-mono text-neon-green/70 uppercase tracking-wider">
              Conversation Log ({logs.length} messages)
            </h3>
            <button onClick={() => refetch()} className="text-gray-500 hover:text-gray-300 p-1">
              <RefreshCw size={12} />
            </button>
          </div>
          ...
          <div 
            ref={logsRef}
            className="log-content bg-black/30 rounded border border-white/5 p-3 max-h-64 overflow-y-auto space-y-2"
          >
            {logs.length === 0 ? (
              <p className="text-gray-600 text-xs">No logs available</p>
            ) : logs.map((log, i) => (
      ...
      {/* Steer panel (only for running agents) */}
        ...
        <div className="p-4 border-t border-white/5">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Send a message to redirect focus..."
              className="flex-1 bg-white/5 border border-white/10 rounded px-3 py-2 text-sm text-gray-200 
                placeholder-gray-600 font-mono"
            />
            <button
              onClick={handleSteer}
              className="btn-neon-blue px-3 py-2 rounded text-sm flex items-center gap-1"
            >
              <Send size={14} />
            </button>
          </div>
...
function LogEntry({ log }) {
  ...
  return (
    <div className="text-xs">
      <span className={`${roleColor} font-bold`}>[{log.role || 'system'}]</span>{' '}
      <span className="text-gray-300">{(log.content || '').slice(0, 500)}</span>
    </div>
  )
}
